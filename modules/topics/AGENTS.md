This module reads the output of the "speech" module and feeds it into an LLM to generate a list of topics and tasks discussed in the audio.

The audio should be split into thematic segments that we will call "topics", each with a start timestamp. The topics will have no end timestamp; it is assumed that a topic ends when the next one starts. Similarly, each topic can have "tasks", with a start timestamp.

The output is a JSON file containing an array of topic objects, each with a "start" field (timestamp in seconds), a "topic" field (text description), and a "tasks" field (array of task objects with "start" and "task" fields).

The module uses ollama python library to interact with configurable local LLM models. The default model is gpt-oss:20b.

The module uses structured output to guide the LLM to produce the desired JSON format. This is an excerpt of the ollama blog post for structured output:
